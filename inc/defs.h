...
// using Bitboard  = uint64_t;
// using Move      = uint16_t;
// using File      = uint_fast8_t;
// using Rank      = uint_fast8_t;
// using Square    = uint_fast8_t;
// using Castle    = uint_fast8_t;
// using Score     = int;

// enum { FILE_A, FILE_B, FILE_C, FILE_D, FILE_E, FILE_F, FILE_G, FILE_H, FILE_num };
// enum { RANK_1, RANK_2, RANK_3, RANK_4, RANK_5, RANK_6, RANK_7, RANK_8, RANK_num };
// enum {  
//     A1, B1, C1, D1, E1, F1, G1, H1, 
//     A2, B2, C2, D2, E2, F2, G2, H2, 
//     A3, B3, C3, D3, E3, F3, G3, H3, 
...
// enum Color  : uint_fast8_t { WHITE, BLACK, COLOR_num };
// enum Piece  : uint_fast8_t { PAWN, KNIGHT, BISHOP, ROOK, QUEEN, KING, PIECE_num };
// enum MoveFlag : uint_fast16_t { 
//     QUIET   = 0b0000'0000'0000'0000, 
//     PUSH    = 0b0001'0000'0000'0000,
//     K_CAST  = 0b0010'0000'0000'0000,
//     Q_CAST  = 0b0011'0000'0000'0000,
...
// constexpr Bitboard bit(int i)                               { return 1ul << i; }
// constexpr File file(Square s)                               { return s & 7; }
// constexpr Rank rank(Square s)                               { return s >> 3; }
// constexpr Square sq(Rank r, File f)                         { return (r << 3) + f; }
// constexpr Square from(Move m)                               { return m & 0x3f; }
// constexpr Square to(Move m)                                 { return m >> 6; }
// constexpr MoveFlag flag(Move m)                             { return MoveFlag(m & 0xf000); }
// constexpr Move mv(Square src, Square dst)                   { return (dst << 6) | src; }
// constexpr Move mv(Square src, Square dst, MoveFlag flag)    { return (dst << 6) | src | flag; }
...
//     return (((bb + (bb >> 4)) & 0xF0F0F0F0F0F0F0F) * 0x101010101010101) >> 56;
// }
// constexpr Square lsb(Bitboard bb)           { return cnt((bb & -bb) - 1); }
// constexpr Color operator~(Color c)          { return Color(c ^ BLACK); }
